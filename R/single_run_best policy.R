...
plot <- ggplot(single_trial_full_data, aes(x = timestep, y = tumor_state)) +
  ...
  geom_point(data = fep_ex_df %>% filter(testing_planned == 1), aes(x = future_timepoint, y = 0, shape = "Planned Testing"), size = 1) +
  scale_shape_manual(values = c("Planned Testing" = 4)) +
  
  
  #draw a vline at timestep 16
  geom_vline(xintercept = 16, linetype = 15, alpha = .5) +
  geom_vline(xintercept = 34, linetype = 15, alpha = .5) +
  ...
  geom_rect(data = fep_ex_df %>% filter(treatment_planned == 1), aes(xmin = future_timepoint, xmax = future_timepoint+1,  ymin = -Inf, ymax = Inf, fill = "Planned Treament"), inherit.aes = FALSE, , alpha = 0.4, label = "Planned Treatment")  +
  ...
  labs(title = "Examples of Best Policies",
       x = "Time",
