...
The clinical trial reported by [@zhang2017] only utilized a single drug, a group of researches Adapative have recently surveyed the use of mathemtical modelling in the field. They have identified a number needed developments. For example, they argue that modelling effect of multidrug therapies will be critical. Additionally, they argue that it is unlikely that any treatment approach can accomplish delaying the emergence of resistant cells, lower the tumor burden and minizmize the toxicity. Given that patients likely differ in their ability to tolerate tumor burden and the toxicity of drugs and the evolutioanry dynamics of the cancers that they carry differ, models would ideally therefore have to trade-off each these factors given a specific patient. Additionally, a modelling should of more complicated tumor dynamics is required. The authors highlight certain biological mechanisms as such as the how competition dynamics changes depending on how close a tumor is to carrying capacity. Likewise high tumor burdens might also come with other costs, such as increasing the risk of new metastasis. Models will therefore have to be able to specify complex causal systems.

Predidict individual patient treatment responses would greatly enhance adaptive therapy since dose modulation could be individualized further and evolutionary dynamics could be controlled with more precision. A key issue going forward is rethinking how data on patients is collected. It will be crucial to collect data not only to detect progression, but to collect data that will be useful for future decisions too. For example, a risk of using a set testing frequency, as done in the pilot clincal trial, is treatment *overshooting*, which limits future opportunities for applying treatment. While modelling these factors among others are likely pivotal to improve adapative therapy, the authors also stress importance carefull analysis of unceartinties involved in these models [@west2023].
...
One approximate solutions, of these is born out of the neuroscience paradigm of *Active Inference*. TBy minimizing two objective functions *Varitional* *Free Energy* (VFE), a measure of a generative model and past sensory inputs, and *Expected Free Energy* (EFE) a score of future courses of actions against a set a set of preferwences. Active Inference has been used to model psychopathology [@dacosta2020] but also applied to control scenarios such as the mountain-car problem [@friston2009] and, albeit augmented with deep-learning, robotics control \@[@çatal2020]. Active Inference implementations of the POMPD scheme have been implemented in MATLAB and recently in Python with the python package *pymdp* [[@heins2022a] and [@heins2022].

Typically Active Inference POMDPs are modelled for discrete state spaces. Mathematically they are described as a joint probability:

$$
p(o,s,u;\phi)
$$

where $o$ are observations, $s$ are hidden states, $u$ are "control states" (states that an agent can influence) and $\phi$ are the hyperparameters of the model, such as $\alpha$ typically used as 'inverse temperature' i.e. how deterministicly the model selects actions. By conditioning on certain observations, the POMDP is solvable by various approximations schemes for the optimal posterior beliefs over what the underlying hidden states are and what the optimal course of action is given a set of preferences. For example, a POMPD could model the joint probability of observing a certain amount of tumor burden in a patient, given some hidden states, such as a tumor burden, a degree of treatment resistnce resistant tumors and whether a certain treatment was applied. Computability is the only limit of far into the future these POMPDs can besolved. Due to considering time and states discrete, this joint probability can be made tractable by factorizing the joint probability into the following categorical probability distributions:
...
The POMPD can be solved at any time step by minimizing two measures, *Variational Free Energy* (VFE) and *Expected Free Energy* (EFE) to yield the optimal beliefs about current states given obervations and the best course of action. VFE is an upper bound on *surprisal,* and is approximated to infer the optimal posterior of current states given some observations, the likelihood model, and the prior over either initial states if no earlier orbsevations have been made. Otherwise the the precedding posterior over states is used as the current prior [@smith2022]. VFE scores how well the models representation aligns with a set of observations, and by using different approximate posteriors over states, choosing the approximate posterior over states which minizimes VFE will approximate bayesian belief updating [@dacosta2020]. EFE instead scores *policies,* plans about future actions, on their expected outcomes. EFE is used to construct a posterior distribution over what policy is most preferrable given $C$, the set of prior preferences over observations.

## The aim of this paper

The treatment decisions used by [@zhang2017] were based on a heuristic about how a biomarker of tumor burden related to resistance dynamics. This paper will investigate whether the Active Inference implenentation of POMDPs could be viable in explicitly modelling the degree of treatment resistance and controlling it in analgous setting. Beyond the desiderata of models reported by [@west2023], using the Active Inference paradgim was further motivated by the following considerations:

-   Real-time data is likely sparse in clinical settings. Extracting the maximum amount of information for each data point will therefore be essential. Using models that perform optimal bayesian inference would therefore be preferable.

-   Short-term exploiting tumor vulnerability is essential for a keeping the patient alive, but for long term success, controlling resistance dynamics will be essential. A therapy plan will have to balance keeping the patient alive now against limiting its future options for treating. Using a measure such as Expected Free Energy could be instrumental to striking this balance.

-   Resistance dynamics are not directly measurable, but should be inferable through observing how the tumor burden responds to treatment. Choosing whether to treat would therefore not only be a consideration of the tumor level, but also about how much wiser it would make us about the resistance dynamics.

-   The information gain from testing can be quantified, and used to prioritize testing for periods of greater uncertainty.

An interesting corollary to the motivations, is that POMPDs could exhibit some counterintuitive suggestions. For example, a model could estimate correctly estimate that one patient has a low tumor burden and suggest that not to apply treatment. The model could propose treating a patient that has exactly the same burden. This is a feature, not a bug. If model has less beliefs about the resistance dynamics of the second patient applying treatment could be the optimal move, simply to gain information about the resistance dynamics.

# Analysis

## Simulated environment

To invesitage the appropriateness of the Active Inference implementation of the POMDP scheme multiple simulations were run. In order to easily comply with the discrete state and time implementation typical of the Active Inference literature, discrete states and time were used. On each simulation run an simulated clinical setting of maximally 200 timesteps was run. In each timestep, a POMPD has to keep a virtual cancer patient alive by applying deciding when to apply a treatment based on a signal of the tumor burden. This enviorment was inspired by the use of PSA testing, and Abiraterone treatment in the the pilot clinical trial [@zhang2017]. In the simulations, a patient's *tumor state* determines whether they survive to the next timestep. The simulation is ended if not. Simulation runs always begin with the tumor state at 0, but it increases with a fixed risk at each timestep. If the tumor state reaches 5, the virtual patient "dies" and the simulation is ended.

Applying treatment can reduce the tumor state, and thus avoid the patient dying. But whether a treatment is successfully reduces the tumor state depends on a underlying *resistance state*, which also also begins at state 0 out of 5. When the resistance is low, the chance of treatment succeeding is high. But for each round of treatment application, the resistance state has a fixed risk of increasing. This renders treatment less and less efficient. The resistance state can decrease if treatment is withdrawn, and the probability of the resistance state decreasing depends on the tumor level. At high tumor levels, the resistance state is the most likely to decrease. This is based on the work of [@hansen2020] who suggests that larger tumor sizes should generate more competitive pressure on resistant-cells.

All in all this means that a model has to balance the tumor state not growing out of control, which would "kill the patient", while not painting itself into a corner by applying treatment too frequently. In order to successfully long-term manage the disease, a model will therefore also have to manage the resistance state.

## Modifications to POMPD scheme

Typically in Active Inference implementations of POMPDs, different types of hidden states such as resistance and tumor states are not modelled to affect each other. Instead the interactions of combinations of different states are modelled to lead to different observations. This means that they would be in the $A$-array, which contains the likelihood mappings.

However, this setup was deemed incompatible with clinical setting that was being modeled. Therefore changes to the implementation in pympd was made to allow that a model to infer an causally upstream occluded states, such as the resistance state, through its down-stream effects partially observable states, i.e. the tumor state. This approach should allow to further complexion models.

Specifically, modifications were made to the structure of the generative model's transition beliefs. Usually, three dimensional "B-tensors" describe expected transition probabilities for a hidden state. This would mean a b-tensor would hold one dimension the current state, one dimension for current state is chosen and third for every allowed action. These b-tensors were given another another dimension that corresponds to the state of type of hidden state. Concretely, this meant that the tumor state factor had a fourth dimension corresponding to tumor transition probabilities for each resistance state. The expected transition probabilities could then be estimated by matrix multiplication of the B-tensor containing the tumor state transition proabilities and a vector of expected resistance level at a given time point. One can imagine that instead of having only one B-tensor for the transition probabilities of the tumor state, multiple b-tensors of tumor transition rates were specified. The transition probabilites of each tensor then corresponded to a specific resistance level. This modification to the generative model neccistates to simply taking an average of all these tensors weighted by the expected probabilities for each resistance state. This all amounts simply calculating the expected tumor state conditional on a set of beliefs about the resistance state and whether treatment was being applied. This modification strategy was repeated for the resistance level, since decreases in the resistance state depended on the tumor state. While these changes rendered much of the functionality pymdp immediately unsuable, this strategy should be able to model arbitrarily complex dependencies between states. Hopefully, a robust implementation can be designed without overhauling pymd. It should also be noted that this modification means that it is longer meaningless in which order beliefs about states are evaluated. Instead this must be specified. For example for the present simulations, the effect of resistance level on the treatment efficacy was evaluated first, since this was evaluated first at at each step of the simulation.
...
A plot of the models beliefs about the resistance state at each time-point (see Fig. 2) also shows that the its beliefs generally follow the development of the actual resistance level.
...
```{r, fig.cap= "Figure 3. Shows the expected trajectories for highest evaluated policy at certain timepoints"}
...
Several modified Active Inference POMPDs were tested against a range-bounded strategy. This was done to test to benchmark the performance of POMPDs against a strategy inspired by treatement protocol used in the pilot clinical trial [@zhang2017]. The POMPDs were changed, so that they only could observe the tumor state, and they would recieve a noiseless signal of this on every timestep. They were also only given prior preferences for orbsevations of tumor states. These were uniform over all except the for highest tumor state, for which the probability was set to 0. Three POMPDs were tested with different lengths of policy horizons:

-   A short horizon model that could consider a block of treating or not treating for the next three steps.

-   A medium horizon model that could consider two blocks of treat tating or not treating for three steps. This yielded a total horizon of 6 timesteps

-   A long horizon model that could consider 4 blocks of treating or treating for three steps, for a total horizon of 12 steps.

These four models, were tested on 4 bouts of different "tumor aggressivity settings". The tested growth rates were .1, .3, 5, and .7 probability of increasing the tumor state at each timestep. Each of these 4 bouts consisted of 100 simulation runs for each model. For each single run, the environment was predetermined by generating a 200 outcomes at each combination of tumor state, resistance state and treatment state. This was done to ensure comparability at the level of each run.
...
On the lowest tumor growth rate almost all the runs reach maximal length (see Fig. 7). The longest policy strategy also appear to perform slightly better than the other models. Since the difficulty of managing the tumor burden changed, contrast are computed between the long policy POMPD and the range-bounded strategy. Since each stochasticity of each run was pretedermined, the a percentage difference from switching from a RB-strategy to POMDP can be computed for each run (see Fig. 8).
...
On the 100 runs of .1 risk of tumor growth, the long horizon POMPD, seems to perform similar to the RB strategy. But as the tumor growth risk increases, the model POMPD increaseingly shows large percentage increases as compared to the RB strategy. This could be beacuse as the tumor dynamics speed up the need for proactive action becomes more important, thus favoring models that attempt to also control the underlying resistance state.
