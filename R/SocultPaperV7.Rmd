...
(see appendix A for an in-depth description of how an agents generative
model is specified by factorizing the joint probability) [@smith2022].
[^1]
...
```{r fig.cap= "A simulation run for exploratory purposes. The simulation ended before time-step 200 since the simulated patient "died" when the tumor state factor reached the state 5."}
...
```{r, fig.cap="Agents beliefs about the patient's resistance  state at every timepoint. Size and color of circles indicate the probability that the agent assigns to each value of the state at a given time-step."}
...
```{r, fig.cap= "Simulated observed patient survival times for each POMPD agent and the range-bounded strategy."}
#print densities
...
Additionally, costs to testing and treating were implemented and could
be quantitatively integrated using free energy minimization to trade-off
utility for reductions in uncertainty. Considering that key AT
reasearches have reported that future models of AT must be able to
balance drug toxicity, tumor burdens and resistance dynamics
[@west2023], seeking inspiration in the AI paradigm is potentially
promising path for future modelling work.
...
detailed models of real-time tumor biology — all of which are desiderata
...
Another quality of the AI paradigm is its ability to quantitavely
incorporate expected utility and unceartinty when selecting actions.
Basing therapy planning in AI models would blur the line between testing
and treating. For example, models could correctly believe that two
patients have exactly the same tumor burden but suggest treatment for
one patient and not the other. This is a feature, not a bug. If a model
is certain in its beliefs about one patient's resistance dynamics, the
expected gain in information will be smaller. Yet if the model is
uncertain about the resistance dynamics of the other patient, it might
be worthwhile to treat simply for the information gain. Curiously, this
could result in two patients with exactly the same tumor burden, but
only one of them should receive treatment. Factoring in current and
expected uncertainty would therefore drastically alter how to optimally
collect real-time data patient data.
...
factored in to decision-making [@smith2022 and @dacosta2020].
...
tumor states for a substantial amount of time. A systematic method for
comparing models could ease comparison of models. Choice of modeling
framework, building a benchmarking could help propel modeling work in
AT. The work of @west2023 could potentially be translated to a set of
