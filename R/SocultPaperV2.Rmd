...
One approximate solutions, of these is born out of the field of neuroscience litterature. The field which has come to be know as active inference suggests that the brain could by using a varitional approach. By minimizing two objective functions. *Free Energy* (EFE) as measure of model and past sensory inputs, and *Expected Free Energy* (EFE) which evaluates future courses of actions against a set preferred observations. Active Inference has been used to model psychopathology [@dacosta2020] but also applied to control scenearious such as the mountain-car problem [@friston2009] and, albeit augmented with deep-learning, robotics control \@[@çatal2020]. These have been implemneted in MATLAB and recently in Python with the python package pymdp.

Typically POMDPs are modelled for discrete state spaces. Mathematically they are described as joint probability:

$$
p(o,s,u;\phi)
$$

where $o$ are observations, \$s\$ are hidden states, \$u\$ are "control states" (states that an agent can influence) and $\phi$ are the hyperparameters of the model, such as \$\\gamma\$ typiccaly used as "temperature" or how strongly action evaluations determine the actually selected action [@heins2022]. For example, a POMPD could the joint probability of observing a certain amount of tumor burden in a patient, given some hidden states, such as the how resistancy of the underlying tumors, and whether a certain treatment was applied. This can be done for any time horizon. Due to considering time and states discrete, this joint probability can be made tractable for a time horizons by factorizing the joint probability into the following categorical probability distributions:

-   A likelihood model $A$. This describes the how observations map to a particular state. For example how a PSA reading could map to an underlying tumor burden. It might be perfect signal of the tumor burden, but it could also be implemented as noised signal.

-   A transition model $B$. It describes the probability of state transitioning to another on any particular timestep. This also encodes how actions are expected to influence transition probabilites. It could describe how a tumor is likely to evolve from one timestep to next depending whether treatments is being applied.
...
**\*\* more information is present when treating** \*\*
...
Since the active inference minizime surprisal, if a capable model could be constructed
...
### Demonstrating the usefullness of this model
...
### evaluating the how well these models will be crucial
